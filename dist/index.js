...
io.on('connection', (socket) => {
    ...
    socket.on('sendmessage', (data) => {
        if (data.file) {
            const matches = data.file.fileData.match(/^data:([A-Za-z-+\/\.]+);base64,(.+)$/);
            try {
        ...
        socket.to(data.room).emit('getmessage', Object.assign({ message: data.message }, (data.file ? { file: `https://erprakash.tech/src/uploads/${data.file.name}` } : {})));
